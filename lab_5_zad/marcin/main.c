...
int max_roznica(int a, int b, int c) { return max((c - b), (b - a)); }
...
int policz(int A[], int B[], int C[], int i, int j, int k, int max_roz) {
...
int max_hotele(int A[], int B[], int C[], int i, int j, int k) {
  ...
  // Wykonam trzy testy najpierw jako element najwiekszy wezmę A[i], następnie
  // B[j] i na konie C[k] W każdym teście rozpatrzę 2 przypadki

  // Najpierw A >= B >= C
  max_roz = max(max_roz, policz(A, B, C, i, j, k, max_roz));
  // A >= C >= B
  max_roz = max(max_roz, policz(A, C, B, i, k, j, max_roz));
  // B >= A >= C
  max_roz = max(max_roz, policz(B, A, C, j, i, k, max_roz));
  // B >= C >= A
  max_roz = max(max_roz, policz(B, C, A, j, k, i, max_roz));
  // C >= A >= B
  max_roz = max(max_roz, policz(C, A, B, k, i, j, max_roz));
  // C >= B >= A
  max_roz = max(max_roz, policz(C, B, A, k, j, i, max_roz));
...
int main() {
  ...
  for (int i = 0; i < n; i++) scanf("%d %d", &hotel[i].typ, &hotel[i].odl);
  ...
  // Można zauważyć, że do policzenia maxa na pewno będziemy korzystać z hoteli
  // typów takich, jak pierwszy i ostatni. Wtedy możemy rozwiązać pierwszą część
  // zadania sprowadzając je do przypadku, w którym mamy tylko trzy typy hoteli:
  // jedynki - typ pierwszy, dwojki - typ drugi, trojki - pozostałe
  // Na początku zbieram dane do structa
  // Dzięki temu mam łatwy dostęp do odległości hotelu od początku autostrady i
  // do typu hotelu.
  ...
  // Może się zdarzyć że jakiś typ hotelu występuje na początku i na końcu,
  // wtedy musimy sprawdzić dwie opcje drugiego elementu
  ...
  if (czy_sprawdzac) {
    ...
    j = 0, k = 0, l = 0;
    int licznik = 0;
    while (ostatni == pierwszy) pierwszy = hotel[++licznik].typ;
  ...
  // jeden - pierwsza wartość, dwa - tablica możliwych wartości drugiego hotelu,
  // trzy - trzecia wartość

  int jeden, typ1, typ2, trzy = -1, typ3 = -1, licznik = 0, licznik2 = 0;
  int *dwa;
  ...
  while (hotel[licznik].typ == hotel[++licznik].typ) {
  ...
  while (1) {
    licznik3 = licznik + 1;
    while (hotel[licznik3].typ == typ3 && licznik3 < n) licznik3++;
    if (licznik3 - 2 <= n)
    ...
    for (int i = 0; i < licznik2; i++) {
      if (typ1 != typ2 && typ2 != typ3 && typ1 != typ3 && nast_typ) {
        min_odl2 = max_roznica(jeden, dwa[i], trzy);
        min_odl = min(min_odl, min_odl2);
      }
    }
